...
const TrackingLogin = () => {
    ...
    const onLogin = (data) => {
        ...
        if (isAuth) {
            toastify({ msg: "Login Successfully.", position: "top-center" });
            setLoading(true);
        } else {
            ...
            <div className="bg-white/15 p-3 flex flex-col items-center gap-y-8 py-8 w-[90%] sm:w-3/4 lg:w-[40%] rounded-md">
                ...
                <form onSubmit={handleSubmit(onLogin)} className="flex justify-center w-3/4 items-center flex-col gap-y-6">
                    ...
                    <div className="flex justify-center gap-x-1 text-[#FFFFFF] text-sm font-poppins font-light not-italic leading-normal items-center">
                        ...
                        <button type="button" className="font-medium hover:underline" onClick={() => path.navigate("/tracking-register")}>Register</button>
                    </div>
                </form>
            </div>
        </main>
