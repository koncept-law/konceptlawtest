...
const UploadFile = ({ progressData, setProgressData }) => {
  ...
  const getFolderList = async () => {
    try {
      setPending(true);
      const response = await axios.get(`https://t.kcptl.in/api/foldersName`, {
        headers: {
  ...
  const checkExcelFileHandler = async () => {
    try {
      ...
      const response = await axios.post(
        "https://t.kcptl.in/api/checkfields",
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("konceptLawToken")}`,
      ...
      if (selectedDocsFile && selectedExcelFile) {
        if (newFolder) {
          const response = await axios.get(
            `https://t.kcptl.in/api/foldersName`,
            {
              headers: {
          ...
          if (!foundFolder) {
            ...
            await axios.post(
              `https://t.kcptl.in/api/updateMongoFolder`,
              {
                newFolder,
                role: userRole,
              },
        ...
        if (
          ...
          const response = await axios.post(
            "https://t.kcptl.in/api/genatedDocsUpload",
            formData,
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem(
          ...
          const intervalId = setInterval(async () => {
            try {
              const response = await axios.post(
                "https://t.kcptl.in/api/progressBarServer",
                {
          ...
          const response = await axios.post(
            "https://t.kcptl.in/api/genratedDocsUploadFirebase",
            formData,
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem(
          ...
          const intervalId = setInterval(async () => {
            try {
              const response = await axios.get(
                "https://t.kcptl.in/api/progressBar",
                {
                  headers: {
                    Authorization: `Bearer ${localStorage.getItem(
                      "konceptLawToken"
