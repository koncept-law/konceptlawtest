...
const EmailTemplateEditor = ({
  ...
  const handleInput = async (e) => {
    ...
    if (e.type === "select") {
        try {
            const response = await axios.post("/campaign/getValueBySingleHeader", {
  ...
  return (
    <>
      <div className="w-full flex flex-col justify-center py-5 items-center">
        <div className="flex w-full justify-center items-center">
          <div className="w-1/2 flex h-48 flex-col gap-y-2 items-start justify-start">
            <h2 className="font-poppins not-italic leading-normal font-semibold">
              Email:
            </h2>
            <div
              ref={messageRef}
              className="border h-full w-[95%] border-solid border-slate-200 rounded-sm p-3"
            ></div>
          </div>
